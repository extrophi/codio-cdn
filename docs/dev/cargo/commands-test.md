<!-- Source: https://doc.rust-lang.org/stable/cargo/commands/cargo-test.html -->
...
cargo-test â Execute unit and integration tests of a package
...
dashes (`--`) are passed to the test binaries and thus to *libtest* (rustcâs
built in unit-test and micro-benchmarking framework). If youâre passing
arguments to both Cargo and the binary, the ones after `--` go to the binary,
the ones before go to Cargo. For details about libtestâs arguments see the
output of `cargo test -- --help` and check out the rustc bookâs chapter on
...
Setting the working directory of tests to the packageâs root directory makes it
possible for tests to reliably access the packageâs files using relative paths,
...
directory of the package the test belongs to, and is controlled via `rustdoc`âs
...
`--package` *spec*â¦
:   Test only the specified packages. See [cargo-pkgid(1)](cargo-pkgid.html) for the
...
* lib â used to link with binaries, examples, integration tests, and doc tests
* bins (only if integration tests are built and required features are
  available)
* examples â to ensure they compile
* lib as a unit test
* bins as unit tests
* integration tests
...
the example as a test, replacing the exampleâs `main` function with the
libtest harness. If you donât want the `main` function replaced, also include
...
`--doc`
:   Test only the libraryâs documentation. This cannot be mixed with other
    target options.
...
the first failure. To âcompileâ as many tests as possible, use `--tests` to
