...
Functions are prevalent in Rust code. Youâve already seen one of the most
...
First the âHello, world!â message prints, and then `another_function` is called
...
projectâs *src/main.rs* file with the preceding example and run it using `cargo run`:
...
statement in itself. (As weâll see below, *calling* a function is not a
statement, though.)
...
youâll write in Rust. Consider a math operation, such as `5 + 6`, which is an
