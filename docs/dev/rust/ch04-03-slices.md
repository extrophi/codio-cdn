...
Hereâs a small programming problem: write a function that takes a string of
...
[âStoring UTF-8 Encoded Text with Stringsâ](ch08-02-strings.html#storing-utf-8-encoded-text-with-strings) section
of Chapter 8.

Letâs work through how weâd write the signature of this function without using
...
The `first_word` function has a parameter of type `&String`. We donât need
...
clear as we keep going.) But what should we return? We donât really have a way
to talk about *part* of a string. However, we could return the index of the end
of the word, indicated by a space. Letâs try that, as shown in Listing 4-7.
...
Weâll discuss iterators in more detail in [Chapter 13](ch13-02-iterators.html).
...
destructure that tuple. Weâll be discussing patterns more in [Chapter
...
![Three tables: a table representing the stack data of s, which points to the byte at index 0 in a table of the string data "hello world" on the heap. The third table rep-resents the stack data of the slice world, which has a length value of 5 and points to byte 6 of the heap data table.](img/trpl04-07.svg)
...
slice. The type that signifies âstring sliceâ is written as `&str`:
...
[âImplicit Deref Coercions with Functions and
Methodsâ](ch15-02-deref.html#implicit-deref-coercions-with-functions-and-methods) section of Chapter 15.
...
part of an array. Weâd do so like this:
