...
![Three tables: the table for s contains only a pointer to the table for s1. The table for s1 contains the stack data for s1 and points to the string data on the heap.](img/trpl04-06.svg)
...
parameterâs scope, but the value pointed to by the reference is not dropped
when `s` stops being used, because `s` doesnât have ownership. When functions
have references as parameters instead of the actual values, we wonât need to
return the values in order to give back ownership, because we never had
ownership.

We call the action of creating a reference *borrowing*. As in real life, if a
person owns something, you can borrow it from them. When youâre done, you have
to give it back. You donât own it.
...
last until itâs used in the `println!`, but between the creation of that
mutable reference and its usage, we tried to create another mutable reference
in `r2` that borrows the same data as `r1`.

The restriction preventing multiple mutable references to the same data at the
same time allows for mutation but in a very controlled fashion. Itâs something
that new Rustaceans struggle with because most languages let you mutate
whenever youâd like. The benefit of having this restriction is that Rust can
...
when youâre trying to track them down at runtime; Rust prevents this problem by
...
Users of an immutable reference donât expect the value to suddenly change out
from under them! However, multiple immutable references are allowed because no
one who is just reading the data has the ability to affect anyone elseâs
reading of the data.
...
created. These scopes donât overlap, so this code is allowed: the compiler can
...
pointer*âa pointer that references a location in memory that may have been
given to someone elseâby freeing some memory while preserving a pointer to that
memory. In Rust, by contrast, the compiler guarantees that references will
never be dangling references: if you have a reference to some data, the
compiler will ensure that the data will not go out of scope before the
reference to the data does.

Letâs try to create a dangling reference to see how Rust prevents them with a
compile-time error:
