...
functions, external crates, and more! In the following chapters, weâll explore
these ideas in more detail. In this chapter, youâll just practice the
fundamentals.
...
As you saw in Chapter 1, `cargo new` generates a âHello, world!â program for
...
Now letâs compile this âHello, world!â program and run it in the same step
...
line. We use the `let` statement to create the variable. Hereâs another example:
...
the [âVariables and Mutabilityâ](ch03-01-variables-and-mutability.html#variables-and-mutability)
...
function of the `String` type. An *associated function* is a function thatâs
implemented on a type, in this case `String`. This `new` function creates a
new, empty string. Youâll find a `new` function on many types because itâs a
...
If we hadnât imported the `io` module with `use std::io;` at the beginning of
...
Weâre also passing `&mut guess` as the argument to `read_line` to tell it what
...
However, one long line is difficult to read, so itâs best to divide it. Itâs
often wise to introduce a newline and other whitespace to help break up long
lines when you call a method with the `.method_name()` syntax. Now letâs
...
`Result`âs variants are `Ok` and `Err`. The `Ok` variant indicates the
...
can use `expect`. Youâll learn about recovering from errors in [Chapter
...
This line prints the string that now contains the userâs input. The `{}` set of
...
weâve been building is a *binary crate*, which is an executable. The `rand`
crate is a *library crate*, which contains code that is intended to be used in
other programs and canât be executed on its own.

Cargoâs coordination of external crates is where Cargo really shines. Before we
...
build again, youâll only see two lines of output:
...
Because the *Cargo.lock* file is important for reproducible builds, itâs often
...
Thereâs a lot more to say about [Cargo](https://doc.rust-lang.org/cargo/) and [its
ecosystem](https://doc.rust-lang.org/cargo/reference/publishing.html), which weâll discuss in Chapter 14, but
...
Letâs start using `rand` to generate a number to guess. The next step is to
...
Next, weâre adding two lines in the middle. In the first line, we call the
...
The second new line prints the secret number. This is useful while weâre
...
fits that armâs pattern. Rust takes the value given to `match` and looks
through each armâs pattern in turn. Patterns and the `match` construct are
powerful Rust features: they let you express a variety of situations your code
might encounter and they make sure you handle them all. These features will be
covered in detail in Chapter 6 and Chapter 19, respectively.

Letâs walk through an example with the `match` expression we use here. Say that
the user has guessed 50 and the randomly generated secret number this time is
38.

When the code compares 50 to 38, the `cmp` method will return
`Ordering::Greater` because 50 is greater than 38. The `match` expression gets
the `Ordering::Greater` value and starts checking each armâs pattern. It looks
at the first armâs pattern, `Ordering::Less`, and sees that the value
...
`guess_str` and `guess`, for example. Weâll cover this in more detail in
...
ânewline.â (On Windows, pressing `enter` results in a carriage return
...
(`:`) after `guess` tells Rust weâll annotate the variableâs type. Rust has a
...
contained `Að%`, there would be no way to convert that to a number. Because it
might fail, the `parse` method returns a `Result` type, much as the `read_line`
method does (discussed earlier in [âHandling Potential Failure with
`Result`â](#handling-potential-failure-with-result)). Weâll treat
this `Result` the same way by using the `expect` method again. If `parse`
returns an `Err` `Result` variant because it couldnât create a number from the
...
The `loop` keyword creates an infinite loop. Weâll add a loop to give users
...
As you can see, weâve moved everything from the guess input prompt onward into
a loop. Be sure to indent the lines inside the loop another four spaces each
and run the program again. The program will now ask for another guess forever,
which actually introduces a new problem. It doesnât seem like the user can quit!

The user could always interrupt the program by using the keyboard shortcut
`ctrl`-`c`. But thereâs another way to escape this insatiable
monster, as mentioned in the `parse` discussion in [âComparing the Guess to the
Secret Numberâ](#comparing-the-guess-to-the-secret-number): if
...
Typing `quit` will quit the game, but as youâll notice, so will entering any
...
type and `Result` is an enum that has the variants `Ok` and `Err`. Weâre using
...
catch-all value; in this example, weâre saying we want to match all `Err`
