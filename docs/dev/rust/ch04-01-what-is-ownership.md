...
any of the rules are violated, the program wonât compile. None of the features
of ownership will slow down your program while itâs running.

Because ownership is a new concept for many programmers, it does take some time
to get used to. The good news is that the more experienced you become with Rust
and the rules of the ownership system, the easier youâll find it to naturally
...
Many programming languages donât require you to think about the stack and the
...
the middle or bottom wouldnât work as well! Adding data is called *pushing
...
someone in your group comes late, they can ask where youâve been seated to
find you.
...
Itâs most efficient to get all the orders at one table before moving on to
...
works on data thatâs close to other data (as it is on the stack) rather than
...
Now that weâre past basic Rust syntax, we wonât include all the `fn main() {`
code in examples, so if youâre following along, make sure to put the following
...
which itâs declared until the end of the current *scope*. Listing 4-1 shows a
...
than those we covered in the [âData Typesâ](ch03-02-data-types.html#data-types) section
...
Weâll concentrate on the parts of `String` that relate to ownership. These
aspects also apply to other complex data types, whether they are provided by
the standard library or created by you. Weâll discuss `String` in more depth in
[Chapter 8](ch08-02-strings.html).
...
`string_from`. Weâll discuss this syntax more in the [âMethod
Syntaxâ](ch05-03-method-syntax.html#method-syntax) section of Chapter 5, and when we talk
about namespacing with modules in [âPaths for Referring to an Item in the
Module Treeâ](ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html) in Chapter 7.
...
* We need a way of returning this memory to the allocator when weâre done with
  our `String`.
...
(GC)*, the GC keeps track of and cleans up memory that isnât being used
anymore, and we donât need to think about it. In most languages without a GC,
itâs our responsibility to identify when memory is no longer being used and to
call code to explicitly free it, just as we did to request it. Doing this
correctly has historically been a difficult programming problem. If we forget,
weâll waste memory. If we do it too early, weâll have an invalid variable. If
we do it twice, thatâs a bug too. We need to pair exactly one `allocate` with
exactly one `free`.
...
[`drop`](../std/ops/trait.Drop.html#tymethod.drop), and itâs where the author of `String` can put
...
We can probably guess what this is doing: âbind the value `5` to `x`; then make
...
it to `s2`. But this isnât quite what happens.
...
capacity matters, but not in this context, so for now, itâs fine to ignore the
capacity.
...
no longer valid. Therefore, Rust doesnât need to free anything when `s1` goes
out of scope. Check out what happens when you try to use `s1` after `s2` is
created; it wonât work:
...
shallow copy, itâs known as a *move*. In this example, we would say that `s1`
...
automatically create âdeepâ copies of your data. Therefore, any *automatic*
...
valueâs memory immediately. Consider this code, for example:
...
![One table s representing the string value on the stack, pointing to the second piece of string data (ahoy) on the heap, with the original string data (hello) grayed out because it cannot be accessed anymore.](img/trpl04-05.svg)
...
Thereâs another wrinkle we havenât talked about yet. This code using
integersâpart of which was shown in Listing 4-2âworks and is valid:
...
types that are stored on the stack, as integers are (weâll talk more about
...
weâll get a compile-time error. To learn about how to add the `Copy` annotation
to your type to implement the trait, see [âDerivable
Traitsâ](appendix-03-derivable-traits.html) in Appendix C.
