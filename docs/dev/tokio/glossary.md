<!-- Source: https://tokio.rs/tokio/glossary -->
...
[`tokio::spawn`](https://docs.rs/tokio/1/tokio/fn.spawn.html) or [`Runtime::block_on`](https://docs.rs/tokio/1/tokio/runtime/struct.Runtime.html#method.block_on) function. Tools for creating futures by
combining them such as `.await` and [`join!`](https://docs.rs/tokio/1/tokio/macro.join.html) do not create new tasks, and each
...
Spawning is when the [`tokio::spawn`](https://docs.rs/tokio/1/tokio/fn.spawn.html) function is used to create a new task. It
can also refer to creating new thread with [`std::thread::spawn`](https://doc.rust-lang.org/stable/std/thread/fn.spawn.html).
...
This uses [the `impl Trait` syntax](https://doc.rust-lang.org/book/ch10-02-traits.html#returning-types-that-implement-traits) to return a future, since
[`Future`](https://doc.rust-lang.org/stable/std/future/trait.Future.html) is a trait. Note that since the future created by an async block does
...
warning)](https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=4faf44e08b4a3bb1269a7985460f1923).
...
To be specific, a future yields whenever it returns from the [`poll`](https://doc.rust-lang.org/stable/std/future/trait.Future.html#method.poll) method.
...
code](https://docs.rs/tokio/1/tokio/#cpu-bound-tasks-and-blocking-code) section from the Tokio API reference.

## Stream

A [`Stream`](https://docs.rs/tokio-stream/0.1/tokio/trait.Stream.html) is an asynchronous version of an [`Iterator`](https://doc.rust-lang.org/stable/std/iter/trait.Iterator.html), and provides a
...
The word stream is confusingly sometimes used to refer to the [`AsyncRead`](https://docs.rs/tokio/1/tokio/io/trait.AsyncRead.html) and
[`AsyncWrite`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWrite.html) traits.

Tokio's stream utilities are currently provided by the [`tokio-stream`](https://docs.rs/tokio-stream) crate.
...
parts. Tokio provides a [number of channels](https://docs.rs/tokio/1/tokio/sync/index.html), each serving a different
purpose.
...
* [broadcast](https://docs.rs/tokio/1/tokio/sync/broadcast/index.html): multi-producer, multi-consumer. Many values can be send. Each
  receiver sees every value.
* [watch](https://docs.rs/tokio/1/tokio/sync/watch/index.html): single-producer, multi-consumer. Many values can be sent, but no
...
each message, you can use the [`async-channel`](https://docs.rs/async-channel/) crate.

There are also channels for use outside of asynchronous Rust, such as
[`std::sync::mpsc`](https://doc.rust-lang.org/stable/std/sync/mpsc/index.html) and [`crossbeam::channel`](https://docs.rs/crossbeam/latest/crossbeam/channel/index.html). These channels wait for messages
...
See [the channels chapter](/tokio/tutorial/channels) for an example of an actor.

[![](/img/arrow-left.svg)Topics](/tokio/topics)

Get Help:

[Edit this page](https://github.com/tokio-rs/website/edit/master/content/tokio/glossary.md)