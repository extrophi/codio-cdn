...
is a trait for reading ([`AsyncRead`](https://docs.rs/tokio/1/tokio/io/trait.AsyncRead.html)) and a trait for writing ([`AsyncWrite`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWrite.html)).
Specific types implement these traits as appropriate ([`TcpStream`](https://docs.rs/tokio/1/tokio/net/struct.TcpStream.html), [`File`](https://docs.rs/tokio/1/tokio/fs/struct.File.html),
[`Stdout`](https://docs.rs/tokio/1/tokio/io/struct.Stdout.html)). [`AsyncRead`](https://docs.rs/tokio/1/tokio/io/trait.AsyncRead.html) and [`AsyncWrite`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWrite.html) are also implemented by a number
...
[`AsyncReadExt`](https://docs.rs/tokio/1/tokio/io/trait.AsyncReadExt.html) and [`AsyncWriteExt`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWriteExt.html).
...
[`AsyncReadExt::read`](https://docs.rs/tokio/1/tokio/io/trait.AsyncReadExt.html#method.read) provides an async method for reading data into a
...
With [`TcpStream`](https://docs.rs/tokio/1/tokio/net/struct.TcpStream.html) instances, this signifies that the read half of the socket is
closed.
...
[`AsyncReadExt::read_to_end`](https://docs.rs/tokio/1/tokio/io/trait.AsyncReadExt.html#method.read_to_end) reads all bytes from the stream until
EOF.
...
[`AsyncWriteExt::write`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWriteExt.html#method.write) writes a buffer into the writer, returning how
...
[`AsyncWriteExt::write_all`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWriteExt.html#method.write_all) writes the entire buffer into the
writer.
...
Additionally, just like `std`, the [`tokio::io`](https://docs.rs/tokio/1/tokio/io/index.html) module contains a number of
helpful utility functions as well as APIs for working with [standard input](https://docs.rs/tokio/1/tokio/io/fn.stdin.html),
[standard output](https://docs.rs/tokio/1/tokio/io/fn.stdout.html) and [standard error](https://docs.rs/tokio/1/tokio/io/fn.stderr.html). For example,
[`tokio::io::copy`](https://docs.rs/tokio/1/tokio/io/fn.copy.html) asynchronously copies the entire contents of a reader
into a writer.
...
To start, we will implement the echo logic using the [`io::copy`](https://docs.rs/tokio/1/tokio/io/fn.copy.html) utility.
...
a simple client like the one found in the documentation for [`tokio::net::TcpStream`](https://docs.rs/tokio/1/tokio/net/struct.TcpStream.html#examples).
...
Any reader + writer type can be split using the [`io::split`](https://docs.rs/tokio/1/tokio/io/fn.split.html) utility.
...
[`into_split`](https://docs.rs/tokio/1/tokio/net/struct.TcpStream.html#method.into_split) which supports handles that can move across tasks at the cost of
only an `Arc`.

Because `io::copy()` is called on the same task that owns the `TcpStream`, we
can use [`TcpStream::split`](https://docs.rs/tokio/1/tokio/net/struct.TcpStream.html#method.split). The task that processes the echo logic in the server becomes:
...
You can find the entire code [here](https://github.com/tokio-rs/website/blob/master/tutorial-code/io/src/echo-server-copy.rs).

## Manual copying

Now let's look at how we would write the echo server by copying the data
manually. To do this, we use [`AsyncReadExt::read`](https://docs.rs/tokio/1/tokio/io/trait.AsyncReadExt.html#method.read) and
[`AsyncWriteExt::write_all`](https://docs.rs/tokio/1/tokio/io/trait.AsyncWriteExt.html#method.write_all).
...
A stack buffer is explicitly avoided. Recall from [earlier](/tokio/tutorial/spawning#send-bound), we noted that
...
Full code can be found [here](https://github.com/tokio-rs/website/blob/master/tutorial-code/io/src/echo-server.rs).

[![](/img/arrow-left.svg)Channels](/tokio/tutorial/channels)

[Framing![](/img/arrow-right.svg)](/tokio/tutorial/framing)

Get Help:

[Edit this page](https://github.com/tokio-rs/website/edit/master/content/tokio/tutorial/io.md)