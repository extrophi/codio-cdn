<!-- Source: https://tokio.rs/tokio/tutorial/async -->
...
The [`std::future::Future`](https://doc.rust-lang.org/std/future/trait.Future.html) trait definition is:
...
The [associated type](https://doc.rust-lang.org/book/ch19-03-advanced-traits.html#specifying-placeholder-types-in-trait-definitions-with-associated-types) `Output` is the type that the future produces once
it completes. The [`Pin`](https://doc.rust-lang.org/std/pin/index.html) type is how Rust is able to support borrows in
`async` functions. See the [standard library](https://doc.rust-lang.org/std/pin/index.html) documentation for more
details.
...
version of Tokio! The full code can be found [here](https://github.com/tokio-rs/website/blob/master/tutorial-code/mini-tokio/src/main.rs).
...
[`Waker`](https://doc.rust-lang.org/std/task/struct.Waker.html) bound to the current task. The [`Waker`](https://doc.rust-lang.org/std/task/struct.Waker.html) has a `wake()` method. Calling
...
> `Send`. Most types are `Send`, but something like [`Rc`](https://doc.rust-lang.org/std/rc/struct.Rc.html) is not. Types
> that can be **concurrently** accessed through immutable references are
> `Sync`. A type can be `Send` but not `Sync` — a good example is
> [`Cell`](https://doc.rust-lang.org/std/cell/struct.Cell.html), which can be modified through an immutable reference, and
...
need to hook our `schedule` function with [`std::task::Waker`](https://doc.rust-lang.org/std/task/struct.Waker.html). The
standard library provides a low-level API to do this using [manual vtable
construction](https://doc.rust-lang.org/std/task/struct.RawWakerVTable.html). This strategy provides maximum flexibility to
implementors, but requires a bunch of unsafe boilerplate code. Instead of using
[`RawWakerVTable`](https://doc.rust-lang.org/std/task/struct.RawWakerVTable.html) directly, we will use the [`ArcWake`](https://docs.rs/futures/0.3/futures/task/trait.ArcWake.html) utility
provided by the [`futures`](https://docs.rs/futures/) crate. This allows us to implement a simple trait to
...
Then implement [`futures::task::ArcWake`](https://docs.rs/futures/0.3/futures/task/trait.ArcWake.html).
...
The `Task::poll()` function creates the waker using the [`ArcWake`](https://docs.rs/futures/0.3/futures/task/trait.ArcWake.html) utility from
...
[`tokio::sync::Notify`](https://docs.rs/tokio/1/tokio/sync/struct.Notify.html) utility. This utility provides a basic task
...
Using [`Notify`](https://docs.rs/tokio/1/tokio/sync/struct.Notify.html), we can implement a `delay` function using
...
[![](/img/arrow-left.svg)Framing](/tokio/tutorial/framing)

[Select![](/img/arrow-right.svg)](/tokio/tutorial/select)

Get Help:

[Edit this page](https://github.com/tokio-rs/website/edit/master/content/tokio/tutorial/async.md)