...
Instead of using `Vec<u8>`, the Mini-Redis crate uses `Bytes` from the [`bytes`](https://docs.rs/bytes/1/bytes/struct.Bytes.html)
...
from the spawning chapter](spawning#send-bound).
...
[`current_thread`](https://docs.rs/tokio/1/tokio/runtime/index.html#current-thread-scheduler) runtime flavor is used, then the mutex will
never be contended.

> The [`current_thread` runtime flavor](https://docs.rs/tokio/1/tokio/runtime/struct.Builder.html#method.new_current_thread) is a lightweight,
...
The [dashmap](https://docs.rs/dashmap) crate provides an implementation of a more sophisticated
sharded hash map. You may also want to have a look at such concurrent hash table
implementations as [leapfrog](https://docs.rs/leapfrog) and [flurry](https://docs.rs/flurry), the latter being a port of Java's
...
deadlock (in case of Send guards). See a full example and more context [in this blog post](https://draft.ryhl.io/blog/shared-mutable-state/).

[![](/img/arrow-left.svg)Spawning](/tokio/tutorial/spawning)

[Channels![](/img/arrow-right.svg)](/tokio/tutorial/channels)

Get Help:

[Edit this page](https://github.com/tokio-rs/website/edit/master/content/tokio/tutorial/shared-state.md)