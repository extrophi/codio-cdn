...
mod tests {
    ...
    #[test]
    fn test_cid_display() {
        let content = b"Test display";
        let cid = ContentId::new(content);
        let display_str = format!("{}", cid);

        assert!(display_str.starts_with("Qm"));
        assert_eq!(display_str, cid.as_str());
    }

    #[test]
    fn test_invalid_cid_format() {
        let result = ContentId::from_str("InvalidCID");
        assert!(matches!(result, Err(CidError::InvalidFormat)));
    }

    #[test]
    fn test_invalid_base58() {
        let result = ContentId::from_str("Qm!@#$%^&*()");
        assert!(matches!(result, Err(CidError::InvalidBase58)));
    }

    #[test]
    fn test_invalid_hash_length() {
        // Create a valid base58 string but with wrong length
        let short_hash = bs58::encode(&[1u8; 16]).into_string();
        let result = ContentId::from_str(&format!("Qm{}", short_hash));
        assert!(matches!(result, Err(CidError::InvalidHashLength)));
    }

    #[test]
    fn test_empty_content() {
        let cid = ContentId::new(b"");
        assert!(cid.as_str().starts_with("Qm"));
        assert!(cid.verify(b""));
    }
    ...
    fn test_large_content() {
        ...
        let cid = ContentId::new(&large_content);

        assert!(cid.as_str().starts_with("Qm"));
        assert!(cid.verify(&large_content));
    }
    ...
    fn test_cid_hash_equality() {
        let content = b"Hash equality test";
        let cid1 = ContentId::new(content);
