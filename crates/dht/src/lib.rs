            ...
            KademliaEvent::OutboundQueryProgressed { result, .. } => {
                match result {
                    QueryResult::GetProviders(Ok(_ok)) => {
                        // Note: GetProvidersOk has different structure in libp2p 0.53
                        // Provider information is available but CID extraction needs rework
                        tracing::debug!("Providers found (GetProvidersOk received)");
                        // TODO: Fix CID extraction from GetProvidersOk
                    }
